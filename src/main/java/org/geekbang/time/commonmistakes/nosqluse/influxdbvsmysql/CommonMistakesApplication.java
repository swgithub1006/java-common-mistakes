...
//    public static final int ROWS = 10000000;
		...
		for (int i = 0; i < loopCount; i++) {
			jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
				...
				public int getBatchSize() {
					return ROWS / loopCount;
				}
			});
		}
		log.info("init mysql finished with count {} took {}ms",
	...
	private void initInfluxDB() {
		long begin = System.currentTimeMillis();
		OkHttpClient.Builder okHttpClientBuilder = new OkHttpClient().newBuilder().connectTimeout(1, TimeUnit.SECONDS)
...
//        try (InfluxDB influxDB = InfluxDBFactory.connect("http://127.0.0.1:8086", "root", "root", okHttpClientBuilder)) {
		try (InfluxDB influxDB = InfluxDBFactory.connect("http://192.168.10.21:8086", okHttpClientBuilder)) {
			...
			IntStream.rangeClosed(1, ROWS)
					.mapToObj(i -> Point.measurement("m").addField("value", ThreadLocalRandom.current().nextInt(10000))
			...
			log.info("init influxdb finished with count {} took {}ms",
					influxDB.query(new Query("SELECT COUNT(*) FROM m")).getResults().get(0).getSeries().get(0)
							.getValues().get(0).get(1),
					System.currentTimeMillis() - begin);
		}
	}
}
//[12:36:43.979] [main] [INFO ] [o.g.t.c.n.i.CommonMistakesApplication:109 ] - init influxdb finished with count 1.0E7 took 122051ms
//[12:47:34.871] [main] [INFO ] [o.g.t.c.n.i.CommonMistakesApplication:84  ] - init mysql finished with count 10000000 took 650891ms
