...
public class UsernameAutoCompleteService {
	...
	@PostConstruct
	public void wrong() {
		userRepository.saveAll(LongStream.rangeClosed(1, 10000).mapToObj(i -> new UserEntity(i, randomName()))
		...
		userRepository.findAll().forEach(userEntity -> {
			...
			for (int i = 0; i < len; i++) {
				...
				autoCompleteIndex.computeIfAbsent(key, s -> new ArrayList<>()).add(new UserDTO(userEntity.getName()));
			}
		});
	...
	public void right() {
		...
		HashSet<UserDTO> cache = userRepository.findAll().stream().map(item -> new UserDTO(item.getName()))
